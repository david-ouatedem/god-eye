...
export default function Dashboard() {
  ...
  // Calculate total vehicle count and weighted average speed
  const totalVehicleCount = useMemo(() => {
    return trafficData.reduce((sum, data) => sum + data.vehicle_count, 0);
  }, [trafficData]);

  const averageSpeed = useMemo(() => {
    if (totalVehicleCount === 0) return 0;
    const totalWeightedSpeed = trafficData.reduce(
      (sum, data) => sum + data.avg_speed * data.vehicle_count,
      0
    );
    return totalWeightedSpeed / totalVehicleCount; // Correct weighted average speed calculation
  }, [trafficData, totalVehicleCount]);
          ...
          <CardContent className="p-0 overflow-hidden">
            ...
            >
              ...
              <Marker position={[4.04505, 9.71891]}>
                <Popup>
                  https://www.openstreetmap.org/copyright A pretty CSS3 popup.{" "}
